...
class CustomDataModule(LightningDataModule):
    def __init__(self, data_path: str = "data/dummy_dataset.json", batch_size: int = 32, num_workers=1,
            task_status="status_noab" # status status_noab pc
            , sep_token=None #"</s>"
    ):
        super().__init__()
        ...
        self.label_names = []
        self.task_name = "dapt-ft_%s_clf" % (task_status.replace('_noab',''))
        self.args = Munch({
            'maxtokens': 500,
            'context_size': 18,
            'unlabeled': True,
    ...
    def prepare_data(self, seq_length=12, utt_length=20, remove_aborted=False):
        ...
        self.label_names = ['Aborted', 'Solved', 'To be tested', 'Out of scope', 'No solution']

        if remove_aborted:
            ## remove aborted conversations
            for split in ['train', 'val', 'test']:
                ds_json[split] = [ v for v in ds_json[split] if v['status'] != 'Aborted' ]
            self.label_names = ['Solved', 'To be tested', 'Out of scope', 'No solution']

        self.num_labels = len(self.label_names)
        if not remove_aborted:
            self.label_dict = { self.label_names[i]:i for i in list(set([line['status'] for i, line in enumerate(ds_json['train'])])) }
            label_distribution = [ self.label_names[el['status']] for el in ds_json['train'] if el['status'] != None]
        else: 
            self.label_status_dict = { self.label_names[i-1]:i for i in list(set([line['status'] for i, line in enumerate(ds_json['train'])  if line['status'] != 'Aborted' ])) }
            label_distribution = [ self.label_names[el['status'] -1] for el in ds_json['train'] if el['status'] != None and el['status'] != 'Aborted']
        ...
        if self.task_status == "pc":
            self.label_names = ["notproblematic", "problematic"]
            self.num_labels = len(self.label_names)
        ...
        def _status2prob(status):
            status = int(status)
            if status <= 1: return self.label_names.index("notproblematic")
            elif status > 1: return self.label_names.index("problematic")
...
class PLBert(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.dense = torch.nn.Linear(self.roberta.config.hidden_size, self.roberta.config.hidden_size)
        self.dense_targettask = torch.nn.Linear(self.roberta.config.hidden_size, self.roberta.config.hidden_size)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self._compute_celoss(y_hat, label, self.num_labels, weighted=True)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        val_loss = self._compute_celoss(logits, batch['labels'], self.num_labels, weighted=True)
    ...
    def test_epoch_end(self, outputs):
        ...
        cr = classification_report(labels.detach().cpu().numpy(), preds.detach().cpu().numpy(), target_names=self.label_names, output_dict=True, digits=4)
        cr['kappa'] = k.cpu().numpy()
        utils.save_classificationreport(cr, '%s/classification_report.tsv' % (self.trainer.logger.log_dir) )
        print(colored("kappa", "green"), k)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Second step for Domain-Adpative Pretraining (DAPT). Reused the MLM second pretraining for a finetuning on Problem Status or problematic conversation identification (PC) tasks ")
...
dm = CustomDataModule(batch_size=16, num_workers=4*AVAIL_GPUS, task_status=args.status, sep_token="</s>", data_path=args.dataset)
dm.prepare_data(utt_length=512, remove_aborted=True)
...
tblogger = TensorBoardLogger("lightning_logs", name=dm.task_name, log_graph=True)
